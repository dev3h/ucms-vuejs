  ...
  >
    <div class="min-h-[40vh]">
      <div class="flex gap-[16px]">
        ...
        />
        <el-button type="primary" size="large" class="btn-basic" @click="searchData()"
          >Search</el-button
        >
      </div>
      <div class="h-full">
        ...
        <div v-loading="loadingMore">
          <el-main
            class="!pt-0 !px-0 mt-3 max-h-[500px] border-t-[2px] border-[#DCDFE6] overflow-y-scroll"
          >
            <ul v-if="listUsers.length > 0" v-infinite-scroll="loadMore" class="list">
              <div
    ...
    <template #footer>
      <span class="dialog-footer flex justify-center items-center">
        <el-button type="info" size="large" class="button-min--width" @click="closeForm">
          Cancel
        </el-button>
        <el-button
          type="primary"
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      await axios
        .post(this.appRoute('admin.api.notification.member-can-add'), params)
        .then(({ data }) => {
