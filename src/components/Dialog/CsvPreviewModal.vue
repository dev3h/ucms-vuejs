  ...
  >
        ...
        <el-table :data="csvContent" @selection-change="handleSelectionChange" ref="table">
          ...
          >
            <template #default="scope">
              <el-form-item
                :prop="`data.${scope.$index}.${header}`"
                :error="getError(`data.${scope.$index}.${header}`)"
                :inline-message="hasError(`data.${scope.$index}.${header}`)"
              >
    ...
    <template #footer>
      <div class="flex justify-center">
        ...
        <el-button :disabled="selectedRows.length === 0" type="primary" @click="doSubmit()">{{
          $t('button.import')
        }}</el-button>
      </div>
    </template>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      const { status, data } = await axios.post(this.appRoute('admin.api.user.import-csv'), {
        data: selectedData
      })
      this.$message({
