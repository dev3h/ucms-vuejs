      ...
      <div class="w-full">
        <div class="flex border">
            ...
            <div class="px-4 py-[6px] max-h-[300px] h-full overflow-y-scroll">
              <el-checkbox-group v-model="dataHandle" class="flex flex-col">
                <el-checkbox
                  ...
                  @change="handleChangeIgnoreItem(item?.id)"
                />
              </el-checkbox-group>
            </div>
          </div>
          <div class="flex-1">
            <div class="h-12 border-b px-4 flex items-center">
              <span
                >{{ dataHandle?.length }} {{ $t('sidebar.permission') }}
                {{ $t('form.ignore') }}</span
              >
            </div>
...
export default {
  ...
  methods: {
    ...
    async getRoleOfUser() {
      try {
        const response = await axios.get(this.appRoute('admin.api.user.show', this.current_id))
        if (response) {
    ...
    async handleIgnorePermission() {
      ...
      const { status, data } = await axios.put(
        this.appRoute('admin.api.user.ignore-permission-for-user-role', this.current_id),
        {
