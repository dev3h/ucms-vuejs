    ...
    <div id="notification-list" class="w-full bg-white px-4 pb-[24px]">
      ...
      <div class="w-full my-[18px]">
        <el-form ref="form" :model="formData" :rules="rules" label-position="top">
          <div class="w-full flex gap-[48px]">
            <div class="min-w-[400px] w-[30%]">
              <el-form-item
                :label="$t('column.publish-at')"
                prop="is_schedule"
                :error="getError('is_schedule')"
                :inline-message="hasError('is_schedule')"
              >
                <div class="ml-[18px]">
              ...
              <div v-if="formData.is_schedule">
                <el-form-item
                  :label="$t('input.publish.start-date')"
                  prop="published_at"
                  :error="getError('published_at')"
                  :inline-message="hasError('published_at')"
                >
                  <el-date-picker
              ...
              <el-form-item
                :label="$t('input.publish.end-date')"
                prop="published_end_at"
                :error="getError('published_end_at')"
                :inline-message="hasError('published_end_at')"
              >
                <el-date-picker
                  v-model="formData.published_end_at"
                  type="datetime"
                  size="large"
                  :placeholder="$t('input.common.select')"
                  format="YYYY/MM/DD HH:mm"
              ...
              <div v-if="formData.sender_type == 2" class="ml-[18px] list-user">
                <el-form-item
                  prop="user_ids"
                  :error="getError('user_ids')"
                  :inline-message="hasError('user_ids')"
                >
            ...
            <div class="flex-1">
              ...
              >
                <CKEditorComponent
                  :contentProp="formData.content"
                  @updateContent="handleInputEditor"
                />
              </el-form-item>
            </div>
          </div>
        ...
        <div class="mt-5 w-full flex justify-center items-center">
          <el-button type="info" size="large" class="button-min--width" @click="goBack()">
            {{ $t('button.cancel') }}
          </el-button>
          <el-button
            :loading="loadingForm"
...
export default {
  ...
  methods: {
    async fetchData() {
      await axios
        .get(this.appRoute('admin.api.notification.show', this.appRoute().params.id))
        .then(({ data }) => {
          let form = data?.data
    ...
    prepareForSubmit() {
      ...
      return {
        action: this.appRoute().params.id
          ? this.appRoute('admin.api.notification.update', this.appRoute().params.id)
          : this.appRoute('admin.api.notification.store'),
        formData
      }
    },
    async submit() {
