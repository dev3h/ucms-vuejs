  ...
  <div>
    ...
    >
      <template #header>
        <DialogHeader
          :title="
            formType === 'permission' ? $t('button.assign-permission') : $t('button.assign-user')
          "
      ...
      <div class="w-full">
        <div class="flex border">
          ...
          <div class="flex-1">
            <div class="h-12 border-b px-4 flex items-center">
              <span
                >{{ permissionChecked?.length }} {{ $t('sidebar.permission') }}
                {{ $t('form.item-added') }}</span
              >
            </div>
            <div class="max-h-[300px] h-full overflow-y-scroll">
              <div
                v-for="permission in permissionChecked"
                :key="permission?.id"
                class="hover:bg-gray-200"
              >
                <div class="flex items-start justify-between py-3 pl-4 pr-3">
                  <div>
                    {{ permission?.label }} ({{ findSystemByPermission(permission?.code) }})
                  </div>
  ...
  methods: {
    ...
    async handleAssignPermission() {
      ...
      const { status, data } = await axios.post(
        this.appRoute('admin.api.role.assign-permission', this.current_id),
